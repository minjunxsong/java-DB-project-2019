...
import java.util.Calendar;            //달력영역 표시 객체
...
public class helptable extends JPanel implements MouseListener {
...
// DB에서 스윙 화면으로 테이블 값 가져오기(select) , 저장하기(insert), 수
	...
	public void paintComponent(Graphics g) {
		//이미지 삽입 
		icon = new ImageIcon("images/도움게시판 창2.png");
	...
	public void select() { // 테이블에 보이기 위해 검색
		String query = "select  동, 호수, 제목 from helptable";
		...
		try {                       //예외처리문 - try,catch,finally
			...
			while (rs.next()) { // 각각 값을 가져와서 테이블값들을
				model.addRow(
						new Object[] {  rs.getString("동"), rs.getString("호수"), rs.getString("제목") });
			}
			...
			try {
				rs.close();
				pstmt.close();
				con.close(); // 객체 생성한 반대 순으로 사용한 객체는 닫기
			} catch (Exception e2) {
			}
		}
	}
...
// 테이블 새로 저장하는 부분
		...
		jBtnSaveRow.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String query = "insert into helptable(동, 호수, 제목) values (?,?,?)";
				try {
...
//물음표가 4개 이므로 4개 각각 입력해줘야한다.
				...
				JTextField txtfield = new helptextarea().textfield;
		...
		jBtnEditRow.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String query = "UPDATE helptable SET 호수=?, 제목=?" + "where 동=?";
				try {
					Class.forName(driver); // 드라이버 로딩
					con = DriverManager.getConnection(url, user, password);
					pstmt = con.prepareStatement(query);
		...
		jBtnDelRow.addActionListener(new ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent e) {
				...
				try {
					String row2 = "select u_dong, 동,rownum from (SELECT *,row_number() over()AS rownum from users,helptable)x WHERE rownum = ? AND 동 = u_dong AND 호수 = u_hosu";
					...
					if (rs.next()) {
						...
						if (row3 == rs.getInt("rownum")) {
							...
							String query = "DELETE FROM helptable WHERE 제목= ?";
							int row = table.getSelectedRow();
							try {
		...
		jBtnwrite.addActionListener(new ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent e) {
				...
				josu = table.getSelectedRow() + 1;
				helpwrite.josu = josu;
				new helpwrite();
				int row = table.getSelectedRow();
				DefaultTableModel model2 = (DefaultTableModel) table.getModel();
				try {
					String count = "UPDATE help_write SET h_조회수 = h_조회수 + 1 " + "WHERE h_제목 = ?";
					Class.forName(driver);
					// DriverManager로부터 커넥션을 얻어오는데 mysql서버 . id, pw 등을 언어온다.
					con = (Connection) DriverManager.getConnection(url, user, password);
					// 커넥션으로부터 실제로 sql쿼리 실행시키기 위한 statement 객체를 얻어온다.
